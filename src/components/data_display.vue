                    ...
                    <template #name="{ text, record }">
                        <div class="editable-cell">
                            <div v-if="editableData[record.key]" class="editable-cell-input-wrapper">
                                <!--编辑单元格时默认显示原内容-->
                                <a-input
                                    v-model:value="editableData[record.key].comment"
...
export default defineComponent({
    ...
    setup() {
        ...
        const state = reactive({
            ...
            pagination: {
                ...
                onChange: (current) => {
                    ...
                    table_content(state.table_id, 10, current).then((response) => {
        ...
        //处理编辑操作
        const edit = (key) => {
            editableData[key] = cloneDeep(state.dataSource.filter((item) => key === item.key)[0])
        }

        //保存编辑内容
        const save = (key) => {
            Object.assign(state.dataSource.filter((item) => key === item.key)[0], editableData[key])
