...
describe('登录：异常处理', () => {
    it('用户名或密码无效', function () {
        ...
        cy.visit('/login')
        cy.get(`#${dom_map.login.username}`).clear().type(username).should('have.value', username)
        cy.get(`#${dom_map.login.password}`).clear().type(password).should('have.value', password)
    ...
    it('使用未经验证的电子邮件地址登录', function () {
        ...
        cy.url().then((el) => {
            if (el.indexOf('develop') > 0) {
                // 服务器
                cy.get(`#${dom_map.login.username}`).clear().type(email).should('have.value', email)
                cy.get(`#${dom_map.login.password}`).clear().type(valid_password).should('have.value', valid_password)
