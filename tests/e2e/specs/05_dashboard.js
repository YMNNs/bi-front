describe('更改仪表盘尺寸', () => {
    it('更改成功', function () {
        cy.login()
        cy.visit('/dashboard')
        cy.get(`#${Cypress.config('dom_map').dashboard.size}`)
            .parent()
            .parent()
            .click()
...
describe('编辑仪表盘', () => {
    it('新增仪表', function () {
        cy.login()
        cy.visit('/dashboard')
        cy.get(`#${Cypress.config('dom_map').dashboard.edit}`).click()
        cy.get(`#${Cypress.config('dom_map').dashboard.add}`).click()
        cy.get(`#${Cypress.config('dom_map').dashboard.select_chart}`).click()
        cy.url().then((el) => {
            const dashboard_info = {}
            if (el.indexOf(Cypress.config('dev_hostname')) >= 0) {
                // 服务器
                dashboard_info.chart_name = Cypress.config('chart_name')
            } else {
                dashboard_info.chart_name = Cypress.config('mock_data').charts[0].chart_name
            }
            cy.contains('.ant-select-item-option-content', dashboard_info.chart_name).click()
            cy.get(`#${Cypress.config('dom_map').dashboard.finish_add}`).click()
            cy.contains('成功')
        })
    })
    ...
    it('编辑仪表', function () {
        ...
        cy.url().then((el) => {
            ...
            if (el.indexOf(Cypress.config('dev_hostname')) >= 0) {
                ...
                dashboard_info.chart_name = Cypress.config('mock_data').charts.find((i) => i.id === 2).chart_name
            }
            // 删除再恢复
            cy.contains('.ant-card-head-title', dashboard_info.chart_name)
                .parents('.ant-card')
                .children('.ant-card-actions')
                .find(`#${Cypress.config('dom_map').dashboard.delete}`)
                .click()
            cy.contains('确 定').click()
            cy.contains('.ant-card-head-title', dashboard_info.chart_name).should('not.exist')
            cy.get(`#${Cypress.config('dom_map').dashboard.reset}`).click()
            // 进入编辑
            cy.contains('.ant-card-head-title', dashboard_info.chart_name)
                .parents('.ant-card')
                .children('.ant-card-actions')
